...
export default function UsersPage() {
  ...
  const [groups, setGroups] = useState<{ id: string; name: string; description: string; member_count: number }[]>([]);
  ...
  const fetchGroups = async () => {
    try {
      const data = await ApiClient.get<{ id: string; name: string; description: string; member_count: number }[]>('/admin/groups');
      setGroups(data);
    } catch (error) {
  ...
  const handleAddToGroup = async (groupId: string) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to add users to group';
  ...
  const handleExport = async () => {
    try {
      const csv = await ApiClient.get<string>('/admin/export/users');
