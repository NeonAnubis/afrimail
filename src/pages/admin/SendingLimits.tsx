...
export const SendingLimits: React.FC = () => {
  ...
  const handleResetLimit = async (userId: string) => {
    try {
      ...
      setToast({ message: error.message || 'Failed to reset limit', type: 'error' });
    }
  };
  ...
  const handleSuspendSending = async (userId: string, reason: string) => {
    try {
      ...
      setToast({ message: error.message || 'Failed to suspend sending', type: 'error' });
    }
  };
  ...
  const handleResumeSending = async (userId: string) => {
    try {
      ...
      setToast({ message: error.message || 'Failed to resume sending', type: 'error' });
    }
  };
  ...
  const handleUpdateLimit = async (userId: string, dailyLimit: number, tierName: string) => {
    try {
      ...
      setToast({ message: error.message || 'Failed to update limit', type: 'error' });
    }
  };
  ...
  const handleResolveViolation = async (violationId: string) => {
    try {
      ...
      setToast({ message: error.message || 'Failed to resolve violation', type: 'error' });
    }
  };
